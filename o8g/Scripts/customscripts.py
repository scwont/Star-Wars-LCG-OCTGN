...
def CustomScript(card, action = 'PLAY'): # Scripts that are complex and fairly unique to specific cards, not worth making a whole generic function for them.
   ...
   if card.name == 'A Journey to Dagobah' and action == 'THWART' and card.owner == me:
   ...
   elif card.model == 'ff4fb461-8060-457a-9c16-000000000236' and action == 'PLAY': #Hoth Wedge
      ...
      if action == 'PLAY': # In this scenario, Champion is attaching to a vehicle the player had targeted before they played him.
         ...
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Champion in it.
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001058': # Tycho Celchu B167/2
      if action == 'PLAY': # In this scenario, Tycho is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Tycho in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001059': # Wes Janson B167/3
      if action == 'PLAY': # In this scenario, Wes is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Wes in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001064': # Han Solo B168/2
      if action == 'PLAY': # In this scenario, Han is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Han in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
      ...
      setGlobalVariable('Engaged Objective',str(card._id))
      notify("{} activates {} and moves the engagement to it".format(me,card))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001526': # Wedge Antilles B242/2
      if action == 'PLAY': # In this scenario, Wedge is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Wedge in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001532': # Nien Nunb B243/2
      if action == 'PLAY': # In this scenario, Nien is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Nien in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001533': # Loyal Co-Pilot B243/3
      if action == 'PLAY': # In this scenario, Pilot is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Pilot in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001538': # Admiral Screed B243/3
      if action == 'PLAY': # In this scenario, Screed is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Screed in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001604': # Howlrunner B255/2
      if action == 'PLAY': # In this scenario, Howlrunner is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Howlrunner in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001605': # Night Beast B255/3
      if action == 'PLAY': # In this scenario, Night Beast is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Night Beast in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))			
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001556': # Jek Porkins B247/2
      if action == 'PLAY': # In this scenario, Jek is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Jek in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
            ...
            TokensX('Put1isEnhancement-isSilent', '', card)
            notify("{} climbs into {}".format(card,vehicle[0]))			
   elif card.model == 'ff4fb461-8060-457a-9c16-000000001653': # Shuttle Pilot B258/3
      if action == 'PLAY': # In this scenario, Shuttle Pilot is attaching to a vehicle the player had targeted before they played him.
         vehicle = findTarget('Targeted-atVehicle-targetAllied-noTargetingError')
         if len(vehicle) > 0: #If the player has targeted a Vehicle we assume they wanted to put Shuttle Pilot in it.
            hostCards = eval(getGlobalVariable('Host Cards'))
            hostCards[card._id] = vehicle[0]._id
            setGlobalVariable('Host Cards',str(hostCards))
            cardAttachementsNR = len([att_id for att_id in hostCards if hostCards[att_id] == vehicle[0]._id])
